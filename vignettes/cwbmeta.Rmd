...
The users can also specify whether to adjust the residuals with CR1 to CR4 matrices when bootstrapping as below. The default value for the `adjust` argument is set to `CR0`.
...
The `cwb()` function runs within a minute even with high number of bootstrap replications. Below is an example with 999 bootstrap replications. 
