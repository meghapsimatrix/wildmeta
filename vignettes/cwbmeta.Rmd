...
library(robumeta)
data("SATcoaching")
SATcoaching <- subset(SATcoaching, !is.na(study_type) & !is.na(hrs) & !is.na(test))
...
CWB takes longer with models fit using `metafor::rma.mv()`. Below is an example:

```{r timing-rma, warning = FALSE}
...
```{r boot-plot, fig.width = 6, fig.height = 2.5}
