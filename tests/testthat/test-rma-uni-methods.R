...
mod_A <- rma.uni(yi = yi ~ 1, vi = vi, data = oswald2013)

mod_B <- rma.uni(yi = yi, mods = ~ 0 + Crit.Cat, vi = vi,
                 data = oswald2013)

mod_C <- rma.uni(yi ~ Crit.Cat + Crit.Domain + IAT.Focus + Scoring, vi = vi,
                 data = oswald2013)

mod_D <- rma.uni(yi = yi, mods = ~ Crit.Domain + IAT.Focus + Scoring, vi = vi,
                 data = oswald2013)

mod_F <- rma.uni(yi ~ Crit.Cat + Crit.Domain + IAT.Focus, vi = vi,
                 data = oswald2013)
...
test_that("get_boot_F_f() works for rma.uni objects.", {
  ...
  expect_equal(
    f_C1(y_boot = y_boot, cluster = get_cluster(mod_C)),
  ...
  expect_equal(
    f_C2(y_boot = y_boot, cluster = get_cluster(mod_C)),
  ...
  expect_equal(
    f_C3(y_boot = y_boot, cluster = get_cluster(mod_C)),
  ...
  expect_equal(
    f_C4(y_boot = y_boot, cluster = get_cluster(mod_C)),
    Wald_test(mod_C, cluster = get_cluster(mod_C),
  ...
  mod_G <- rma.uni(yi = yi_boot, mods = ~ Crit.Cat + Crit.Domain + IAT.Focus, vi = vi,
                   data = oswald2013)
  ...
  F_stat <- Wald_test(mod_F, cluster = get_cluster(mod_F),
                      constraints = Cmat_G, vcov = "CR0", test = "HTZ")$Fstat
  G_stat <- get_boot_F_f(mod_G, C_mat = Cmat_G, type = "CR0", test = "HTZ")(y_boot = y_boot, cluster = get_cluster(mod_G))
  H_stat <- get_boot_F_f(mod_H, C_mat = Cmat_G, type = "CR0", test = "HTZ")(y_boot = y_boot, cluster = get_cluster(mod_H))
...
test_that("run_cwb options work for rma.uni objects.", {
  ...
  boot_yi_C <- run_cwb(mod_C, cluster = get_cluster(mod_C), R = 3,
                       auxiliary_dist = "Rademacher")
...
test_that("Wald_test_cwb() results do not depend on sort order.", {
  ...
  scram_A <- rma.uni(yi = yi, vi = vi,
                     data = oswald_scram)
  compare_mod_results(mod_A, scram_A, ord)

  scram_B <- rma.uni(yi ~ 0 + Crit.Cat, vi = vi,
                     data = oswald_scram)
  compare_mod_results(mod_B, scram_B, ord)

  scram_C <- rma.uni(yi ~ Crit.Cat + Crit.Domain + IAT.Focus + Scoring, vi = vi,
                     data = oswald_scram)
  compare_mod_results(mod_C, scram_C, ord)
...
test_that("Wald_test_cwb() works when rma.uni uses subset.", {

  oswald_sub <- subset(oswald2013_full, Crit.Cat == "Microbehavior")

  mod_full <- rma.uni(yi = yi, vi = vi,
                      mods = ~ 0 + IAT.Focus + Crit.Domain,
                      data = oswald_sub)

  mod_sub <- rma.uni(yi = yi, vi = vi,
                     mods = ~ 0 + IAT.Focus + Crit.Domain,
...
test_that("Wald_test_cwb() works when rma.uni has missing observations.", {
  ...
  mod_complete <- rma.uni(yi = yi, vi = vi,
                          mods = ~ 0 + IAT.Focus + Crit.Domain,
...
test_that("Wald_test_cwb() works with user-weighted rma.uni models.", {

  mod_wt1 <- rma.uni(yi ~ 0 + Crit.Cat + Crit.Domain + IAT.Focus + Scoring,
                     vi = vi, weights = wt,
                     data = oswald2013)
  ...
  mod_wt2 <- rma.uni(yi ~ 0 + Crit.Cat + Crit.Domain + IAT.Focus + Scoring,
                     vi = vi, weights = wt,
                     data = oswald2013)
