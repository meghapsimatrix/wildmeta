...
# estimate_null() works for rma.mv objects.
...
# get_cluster() works for rma.mv objects.
...
tinytest::expect_equal(get_cluster(mod_C1), study_fac)
tinytest::expect_equal(get_cluster(mod_C2), study_fac)
...
# get_boot_F() works for rma.mv objects.
...
# run_cwb options work for rma.mv objects.
...
compare_mod_results <- function(mod, scram, ord = 1:length(get_res(mod)), tol = 1e-6) {
  tinytest::expect_equal(coef(mod), coef(scram), tolerance = tol)
  tinytest::expect_equal(as.numeric(get_res(mod)[ord]), as.numeric(get_res(scram)), tolerance = tol)
  tinytest::expect_equal(as.numeric(get_fitted(mod)[ord]), as.numeric(get_fitted(scram)), tolerance = tol)
  tinytest::expect_equal(get_cluster(mod)[ord], get_cluster(scram))
}
...
# Wald_test_cwb() works when rma.mv uses subset.
...
tinytest::expect_equal(attr(test_full, "original"), attr(test_sub, "original"))
tinytest::expect_equal(attr(test_full, "bootstraps"), attr(test_sub, "bootstraps"))


# Wald_test_cwb() works with user-weighted rma.mv models.

if (packageVersion("clubSandwich") >= '0.5.5.9999') {
  ...
  tinytest::expect_inherits(test_wt1, "Wald_test_wildmeta")
  ...
  tinytest::expect_inherits(test_wt2, "Wald_test_wildmeta")

  tinytest::expect_equal(test_wt1, test_wt2)
